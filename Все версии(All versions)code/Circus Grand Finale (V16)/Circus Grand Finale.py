                ...
                if controller_ptr == local:
                    if antiotd_obith == 1 and not keyboard.is_pressed(aim) and not potok_ogon.is_set() and win32api.GetAsyncKeyState(0x01) < 0: #Антиотдача на обычные выстрелы
                        if controller_ptr == local and int(ent.detect_vistrela) > 1:
                            delta_x = (ent.otd['y'] - old_punch_y) * nast_antiotd_obith / 1.5
                        ...
                        elif controller_ptr == local and int(ent.detect_vistrela) < 2:
                            old_punch_x = 0.0
                            old_punch_y = 0.0
                ...
                if ent.wts(view_matrix) and ent.health > 0 and not ent.dormant and controller_ptr != local:
                    ...
                    if (ent.team != 2 and kom == 'kt' or ent.team == 2 and kom == 't') or kom == 't_kt':
                        ...
                        if (keyboard.is_pressed(aim)) and im_thel == f"{ent.name}" and math.sqrt((thenter_x - x_gol) ** 2 + (thenter_y - y_gol) ** 2) < radius_poika:
                            ...
                            if aim_ogon == 1 and rast_do_gol < (head - center * 7.6 if head - center * 7.6 > 2 else 2) and not potok_ogon.is_set():                                
                                threading.Thread(target=Ogon, daemon=True).start()
...
def update_interface(): #Бля я хуй знает зачем это, на всякий случай трогать не буду :)
...
button_language = ctk.CTkButton(frame, image=language_image_ru, text="", width=20, height=20, fg_color="transparent", command=lambda: print("Клик!"))
...
hitt_box_telo = ctk.CTkCheckBox( frame_seting, text="Включить бокс тела. (Просто включает прямоугольник тела)")
hitt_box_telo.place(x=20,y=80)
if hit_box_telo == "1":
...
lev_trig = ctk.CTkCheckBox( frame_seting,
                            text="Альтернативный триггер (Только на врагов в прямой видимости)")
lev_trig.place(x=20,y=110)
if lev_triger == "1":
    lev_trig.select()
else:
    lev_trig.deselect()
...
label_seting4.place(x=20,y=200)

nast_antiotd_ob = ctk.CTkEntry(frame_seting)
nast_antiotd_ob.place(x=418,y=200)
nast_antiotd_ob.configure (width=65)
nast_antiotd_ob.insert(0, nast_antiotd_obith)

#Отделяющая полоска
label_seting0 = ctk.CTkLabel( frame_seting, text = "-"*114 )
label_seting0.place(x=20,y=230)

#Имя группы настроек другое
label_seting1 = ctk.CTkLabel(frame_seting, text = "Прочие настройки" )
label_seting1.place(x=175,y=245)

#Ограничение фпс чита
label_seting5 = ctk.CTkLabel(frame_seting, text = "FPS лок чита(НЕ ДОЛЖЕН БЫТЬ ВЫШЕ ЧЕМ В ИГРЕ! ВАЖНО ДЛЯ AIM):" )
label_seting5.place(x=20,y=275)

FPS_t = ctk.CTkEntry(frame_seting)
FPS_t.place(x=452,y=275)
