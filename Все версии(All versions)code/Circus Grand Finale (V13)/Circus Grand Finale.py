                ...
                if ent.wts(view_matrix) and ent.health > 0 and not ent.dormant:
                    ...
                    if (ent.team != 2 and kom == 'kt' or ent.team == 2 and kom == 't') or kom == 't_kt':
                        ...
                        if aim_telo == True or (aim_telo_50 == True and int(f"{ent.health}") < 50):
                            thenter_y2 = thenter_y - (head - center * 6)#Поправка для наводки на тело
                        else:
                            thenter_y2 = int(thenter_y + (head - center * 7.8)) #Поправка чтобы он наводился не на нос а на глаза (мне кажется так лучше регать будет)
                        ...
                        #Аим                
                        if (keyboard.is_pressed(aim)) and im_thel == f"{ent.name}" and math.sqrt((thenter_x - ent.head_pos2d["x"]) ** 2 + (thenter_y - ent.head_pos2d["y"]) ** 2) < radius_poika:
                            x_gol_rast = thenter_x - ent.head_pos2d["x"]
                            y_gol_rast = thenter_y2 - ent.head_pos2d["y"]
                            x_gol = ent.head_pos2d["x"]
                            y_gol = ent.head_pos2d["y"]
