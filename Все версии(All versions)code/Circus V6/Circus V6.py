        ...
        while pm.overlay_loop() and stop == True:
            ...
            for ent in self.it_entities():
                if ent.wts(view_matrix) and ent.health > 0 and not ent.dormant:
                    ...
                    if ent.team != 2 and kom == 'kt' or ent.team == 2 and kom == 't':
                        ...
                        #Фильтрация челов
                        if ((thenter_x + radius_poika > x > thenter_x - radius_poika) and (thenter_y + radius_poika > y > thenter_y - radius_poika)) and im_thel == "q":
                            im_thel = f"{ent.name}"

                        if (thenter_x + radius_poika > x > thenter_x - radius_poika) and (thenter_y + radius_poika > y > thenter_y - radius_poika):
                        ...
                        if (keyboard.is_pressed(aim)) and thel2 == 1:
                            #ускорение доводки
                            ...
                            if thenter_x + radius_poika > x > thenter_x - radius_poika:                 
                                if y > thenter_y < (y + y_aim) - y_aim / 3 and y < thenter_y + radius_poika :
                                     win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, skorost_aim2, 0, 0)
                                elif y < thenter_y > (y + y_aim) - y_aim / 3 and thenter_y - radius_poika < (y + y_aim) - y_aim / 3 :
                                     win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, -(skorost_aim2), 0, 0)

                            if thenter_y + radius_poika > y > thenter_y - radius_poika:                 
                                if x + (x2 / 3) > thenter_x < (x + x2) - x2 / 3 and x + (x2 / 3) < thenter_x + radius_poika:
                                     win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, skorost_aim2, 0, 0, 0) 
                                elif x + (x2 / 3) < thenter_x > (x + x2) - x2 / 3 and thenter_x - radius_poika < (x + x2) - x2 / 3:
                                     win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, -(skorost_aim2), 0, 0, 0)
                            #тригерр       
                            if aim_ogon == 1:
                                if not( y > thenter_y < (y + y_aim) - y_aim / 3 or y < thenter_y > (y + y_aim) - y_aim / 3) :
...
label1.place(x=5, y=115)

entry6 = Entry(root, font=1)
entry6.place(x=460, y=115)
entry6.config (width=1)
entry6.insert(0, skorost_aim)

#ФПС чита
label2 = Label(root, text = "Лок FPS чита,влияет на скорость AIM в идеале 250+,если не тянет снижай графон:" )
label2.place(x=5, y=188)
entry2 = Entry(root, font=1)
entry2.place(x=465, y=188)
...
label3 = Label(root, text = "Множ.ускорения наведения:         Зона торможения наведения в пикс:" )
label3.place(x=5, y=140)

entry3 = Entry(root, font=1)
entry3.place(x=170, y=140)
entry3.config (width=1)
entry3.insert(0, mnosz_usk_navodki)

entry4 = Entry(root, font=1)
entry4.place(x=400, y=140)
entry4.config (width=1)
