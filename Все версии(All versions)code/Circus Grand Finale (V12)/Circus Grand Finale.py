...
def get_vk_code(char):
    return win32api.VkKeyScan(char) & 0xFF

def deagonal_cont_streif(klav1, klav2, kt_klav1, kt_klav2):
    klav1, klav2, kt_klav1, kt_klav2 = map(get_vk_code, [klav1, klav2, kt_klav1, kt_klav2])
    
    win32api.keybd_event(klav1, 0, win32con.KEYEVENTF_KEYUP, 0)
    win32api.keybd_event(klav2, 0, win32con.KEYEVENTF_KEYUP, 0)
    time.sleep(0.025)
    win32api.keybd_event(kt_klav1, 0, 0, 0)
    time.sleep(0.08)
    win32api.keybd_event(kt_klav1, 0, win32con.KEYEVENTF_KEYUP, 0)
    win32api.keybd_event(kt_klav2, 0, 0, 0)
    time.sleep(0.08)
    win32api.keybd_event(kt_klav2, 0, win32con.KEYEVENTF_KEYUP, 0)
    vistrell()
    
    if win32api.GetAsyncKeyState(klav1) and win32api.GetAsyncKeyState(klav2):
        win32api.keybd_event(klav1, 0, 0, 0)
        win32api.keybd_event(klav2, 0, 0, 0)
    
    time.sleep(zad_m_vistri)

def cont_streif(klav, kt_klav):
    klav, kt_klav = map(get_vk_code, [klav, kt_klav])
    
    win32api.keybd_event(klav, 0, win32con.KEYEVENTF_KEYUP, 0)
    time.sleep(0.02)
    win32api.keybd_event(kt_klav, 0, 0, 0)
    time.sleep(0.08)
    win32api.keybd_event(kt_klav, 0, win32con.KEYEVENTF_KEYUP, 0)
    vistrell()
    
    if win32api.GetAsyncKeyState(klav):
        win32api.keybd_event(klav, 0, 0, 0)
                ...
                if ent.wts(view_matrix) and ent.health > 0 and not ent.dormant:
                    ...
                    if (ent.team != 2 and kom == 'kt' or ent.team == 2 and kom == 't') or kom == 't_kt':
                        ...
                        if (keyboard.is_pressed(aim)) and im_thel == f"{ent.name}":
                            ...
                            #Доп ускорение в близи

                            
                            if ((width - center)/((thenter_x*2) / 1920))/2 < 7:
                                skorost_aim2 = skorost_aim * mnosz_usk_navodki
